   ...
   class SocialLogin extends Component {
      static propTypes = {
         ...
         redirect: (props, propName, componentName) => {
            if (
               props.provider === "instagram" &&
               (!props[propName] || typeof props[propName] !== "string")
            ) {
               return new Error(
                  `Missing required \`${propName}\` prop of type \`string\` on ${componentName}.`
               );
            }
         },
      ...
      constructor(props) {
         ...
         this.fetchProvider =
            props.provider === "instagram" || props.provider === "github";
      ...
      componentDidMount() {
         ...
         this.loadPromise = this.sdk
            .load({ appId, redirect, gatekeeper, scope })
            .then(accessToken => {
               if (autoCleanUri) {
                  cleanLocation();
               }
