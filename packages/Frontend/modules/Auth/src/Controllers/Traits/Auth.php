...
namespace Frontend\Auth\Controllers\Traits;
...
trait Auth {
    ...
    public function build_session() {
        if (empty(\Auth::guard(Config::get("packages.frontend.auth.guard", "frontend"))->check())) return false;
        $user = \Auth::guard(Config::get("packages.frontend.auth.guard", "frontend"))->getUser();
        $acl_list = array();
        if(!empty($user->roles)) {
        ...
        request()->session()->put("auth_frontend", $user);
