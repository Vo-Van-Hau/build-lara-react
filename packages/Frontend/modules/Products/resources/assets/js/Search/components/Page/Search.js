...
const SearchPage = ({keyID, q, ...props}) => {

    const { setRouter, data, get_products, get_product_categories } = useContext(SearchContext);
    ...
    return (<>
        <Breadcrumb style={{ padding: '12px' }}>
            ...
            <Breadcrumb.Item>{q ? q.trim() : ``}</Breadcrumb.Item>
        </Breadcrumb>
        <Row className='products_by_category_container' gutter={24}>
            ...
            <Col className='rightSide' span={19}>
                <Title level={5}>Kết quả tìm kiếm cho `{q ? q.trim() : ``}`</Title>
