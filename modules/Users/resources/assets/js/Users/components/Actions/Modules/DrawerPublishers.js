...
const DrawerPublishers = ({visible, setVisible, selectedRowKeys, setSelectedRowKeys}) => {
    const { data, getPublishers } = useContext(UsersContext);
    const { config } = data;
    const { status } = config;
    ...
    const getlist = (page, keySearch) => {
        setLoading(true);
        getPublishers(page, keySearch).then((res) =>{
            let result= res.data;
            ...
            render: (_, record) => {
                return (
                    <Space direction="vertical" size="small">
                        <Tooltip placement="topLeft" title={record.name}>Publishers: {record.name}</Tooltip>
                    </Space>
                )
            }
        },{
